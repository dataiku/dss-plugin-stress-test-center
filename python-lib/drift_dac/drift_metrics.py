...
def domain_classifier_accuracy(X_src, y_src, X_tar, y_tar, dc_model=DomainClassifierModel.RF, return_model=False):
    ...
    model, score, (X_tr_dcl, y_tr_dcl, y_tr_old, X_te_dcl, y_te_dcl, y_te_old) = \
        domain_clf.build_model( [X_src, X_tar], [y_src, y_tar])
...
def compute_drift_metrics(X_src, y_src, predictions_src, X_tar, y_tar, predictions_tar, model,
    ...
    dc_out = domain_classifier_accuracy(X_src_pproc, y_src, X_tar_pproc, y_tar, dc_model, return_dc_model)
    if return_dc_model:
